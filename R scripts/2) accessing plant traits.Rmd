...
seed_dry_mass_TRY_clean <- seed_dry_mass_TRY %>%
  ...
  rename(dry_seed_mass_mg = StdValue, binomial_name = AccSpeciesName) %>%
...
seed_dry_mass_TRY_clean <-  seed_dry_mass_TRY_clean %>%
  ...
  summarise(reference_TRY = str_c(unique(Reference[!is.na(Reference)]), collapse = ", "),
    .groups = "drop")
...
merged_seed_mass <- left_join(species_list, seed_dry_mass_TRY_clean, by = c("binomial_name" = "binomial_name")) %>%
  select(binomial_name, family, genus, species, history_at_IF_site, history_at_CP_site, species_origin, logging_status, mean_dry_seed_mass_mg_TRY, reference_TRY) %>%
  distinct()
...
average_seed_mass_BIEN <- average_seed_mass_BIEN %>%
  ...
  mutate(reference_BIEN = "BIEN") %>%
  select(binomial_name, mean_dry_seed_mass_BIEN, level_used, reference_BIEN)
...
#   group_by(binomial_name, family, genus, species, species_origin, logging_status, history_at_IF_site, history_at_CP_site, level_used, reference_TRY, reference_BIEN) %>%
#   summarise(
#     mean_dry_seed_mass_mg = coalesce(mean_dry_seed_mass_mg_TRY, as.numeric(mean_dry_seed_mass_BIEN)),
#     .groups = "drop"
#   ) %>%
#   mutate(
#     reference = coalesce(reference_TRY, reference_BIEN)
#   ) %>%
...
final_seed_mass <- merged_seed_mass %>%
  ...
  group_by(binomial_name, family, genus, species, species_origin, logging_status, 
           history_at_IF_site, history_at_CP_site, level_used, reference_TRY, reference_BIEN) %>%
  summarise(
    mean_dry_seed_mass_mg = coalesce(mean_dry_seed_mass_mg_TRY, as.numeric(mean_dry_seed_mass_BIEN)),
    .groups = "drop"
  ) %>%
  mutate(
    reference_seed_mass = coalesce(reference_TRY, reference_BIEN),
    level_used = case_when(
      is.na(reference_seed_mass) ~ NA_character_,           # If reference is NA → level_used = NA
      reference_seed_mass != "BIEN" ~ "Species",             # If reference is not BIEN → Species
      TRUE ~ level_used                                       # Otherwise keep existing value
    ) 
  ) %>%
      select(-reference_TRY, -reference_BIEN)
...
merged_seed_length <- left_join(species_list, average_seed_length, by = c("binomial_name" = "species")) %>%
  select(binomial_name, family, genus, species, history_at_IF_site, history_at_CP_site, species_origin, logging_status, mean_value, level_used) %>%
