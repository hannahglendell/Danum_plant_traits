...
wood_densities_merged <- left_join(wood_densities, species_list, by = c("binomial_name" = "binomial_name")) %>%
  select(binomial_name, mean_wood_density_g_cm_3, family, genus, species, species_origin, logging_status)

#I don't think there is much point in combining this with TRY wood densities, because almost all come from this dataset anyway. maybe this is something to come back to later.
```
...
ggplot(wood_densities_merged, aes(x = logging_status, y = mean_wood_density_g_cm_3)) +
...
TALLO - TREE HEIGHT, CROWN RADIUM, STEM DIAMETER
```{r}
#Tallo (tree height)
