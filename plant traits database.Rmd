...
#change the names of the columns so that they are written with capital letters
...
```{r}
#now I want to join the  final_species list and the INDFORSUS species lists together
#I have a dataset called final_species_list which has the columns binomial_name, species_origin and logging_status. I have a dataset called INDFORSUS_species_list. in both datasets there is a column called binomial_name which contains the name of the species. I want to join the two datasets according to the binomial_name.
#
#
all_species_list <- bind_rows(final_species_list, INDFORSUS_species_list) %>%
  group_by(binomial_name) %>%
  summarize(
    Family = first(Family),
    Genus = first(Genus),
